...
export default function EmployerPaymentSettingsScreen({ navigation }) {
  ...
  const onSave = async () => {
    setSaving(true);
    try {
      await userApi.upsertUser({ payment: { bank, accountName, accountNumber } });
      Alert.alert('บันทึกแล้ว', 'อัปเดตการชำระเงินเรียบร้อย');
      navigation.goBack();
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}> 
        ...
        <Text weight={700} style={styles.title}>การชำระเงิน</Text>
        <View style={{ width: 36 }} />
      </View>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text style={styles.label}>ธนาคาร</Text>
        <TextInput style={styles.input} value={bank} onChangeText={setBank} placeholder="เช่น กสิกรไทย" placeholderTextColor="#94a3b8" />
        <Text style={styles.label}>ชื่อบัญชี</Text>
        <TextInput style={styles.input} value={accountName} onChangeText={setAccountName} placeholder="ชื่อ-นามสกุล" placeholderTextColor="#94a3b8" />
        <Text style={styles.label}>เลขที่บัญชี</Text>
        <TextInput style={styles.input} value={accountNumber} onChangeText={setAccountNumber} keyboardType="numeric" placeholder="xxxxxxxxxx" placeholderTextColor="#94a3b8" />
