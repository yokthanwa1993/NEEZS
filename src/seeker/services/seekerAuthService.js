...
async function ensureSeekerRole(user) {
  // Prefer server-side enforcement via backend API
  try {
    await userApi.ensureRole('seeker');
  } catch (e) {
    // Fallback: client-side write if backend not running
    ...
    try {
      ...
      if (!snap.exists()) {
        ...
        await setDoc(ref, { roles: { seeker: true }, lastRole: 'seeker' }, { merge: true });
      }
    } catch {}
  }
  try { await AsyncStorage.multiSet([['NEEZS_ROLE','seeker'], ['NEEZS_PORTAL','seeker']]); } catch {}
}

export async function signInWithEmail(email, password) {
  const cred = await signInWithEmailAndPassword(auth, email, password);
  await ensureSeekerRole(cred.user);
...
export async function signInWithGoogle() {
  ...
  await ensureSeekerRole(userCred.user);
